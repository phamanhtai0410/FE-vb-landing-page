...
const ModalAddLiquidity = () => {
    ...
    const { dataToken, accountBalance, accountApprove, accountStableDebtApprove, accountVariableDebtApprove, errorCode, message, transaction, pending, isOpen } = useSelector(state => state.poolReducer, shallowEqual);
    ...
    const closeModal = () => {
        dispatch({
            type: marketplaceConstants.MODAL_CLOSE_ADD_LIQUIDITY_MARKET
        })
        if (transaction) {
